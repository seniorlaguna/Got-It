...
class BarcodeScannerDialog extends StatefulWidget {
  ...
  const BarcodeScannerDialog(this._width, this._height, this._appBarHeight, this._onBarcodeDetected, {Key key}) : super(key: key);
...
class _BarcodeScannerDialogState extends State<BarcodeScannerDialog> {
  ...
  void onQRViewCreated(BuildContext context, QRViewController qrViewController) {
  ...
  Widget getCameraPreview(BuildContext context) {
    ...
    return Stack(
        children: [
          LastQrScannerPreview(onQRViewCreated: (QRViewController controller) => onQRViewCreated(context, controller)),
          Positioned(
              bottom: 8,
              left: 8,
              child: GestureDetector(child: Icon(Icons.cancel_outlined, size: 40, color: Colors.white,), onTap: () => onCancel(context))
          ),
          Align(
            alignment: Alignment(0, -0.9),
            child: Text(FlutterI18n.translate(context, "product.scan_barcode"), style: TextStyle(
